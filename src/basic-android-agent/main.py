...
def main():
    ...
    with AndroidVisionAgent() as agent:
        ...
        print("-" * 30)
        try:
            has_gmail = agent.get('Does the screen contain the text "Gmail"?', response_schema=bool)
        ...
        try:
            agent.act("Search for the company AskUI in the browser, and open the first result.")
